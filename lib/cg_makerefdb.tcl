...
proc makerefdb_job {args} {
	...
	job reg_${build}_homopolymer -deps {
		...
		file_write [gzroot $target].info [deindent [subst {
			= homopolymer: homopolymer regions of 6+ same bases =
			...
			== Description ==
			Generated by genomecomb by extracting all regions with 6 or more consequtive same bases.
		}]]
	...
	job gene_${build}_intGene -deps $intdeps -targets {
		...
		set genesets {}
		foreach dep $deps {
			set geneset [file_rootname $dep]
			regsub ^gene_ $geneset {} geneset
			regsub ^${build}_ $geneset {} geneset
			lappend genesets $geneset
		}
		file_write [gzroot $target].info [deindent [subst {
			= inGene: integrated geneset containing transcripts from: $genesets =
			...
			== Description ==
			In intGene genomecomb takes transcripts/isoforms from all given genesets ($genesets) and integrates them into one gene annotation
			file (transcriptlist in tsv genepred format). For transcripts given in multiple source genesets, the id in the first 
			geneset in the list that contains the transcript is used.
		}]]
