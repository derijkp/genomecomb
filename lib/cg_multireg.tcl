...
proc multireg_job {compar_file regfiles {limitreg {}}} {
	...
	if {$len <= $maxfiles} {
		...
		job multireg-[job_relfile2name $compar_file] -force $jobforce -deps $files -targets {$target} -vars {isreg limitreg} -code {
			if {$limitreg ne ""} {
				set templist {}
				foreach file $deps {
	...
	while 1 {
		if {$len <= $maxfiles} {
			set target $compar_file
			job multireg-[job_relfile2name $compar_file] -force $jobforce -deps $todo -targets {
				$target
			} -vars {
		...
		while {$pos < $len} {
			...
			if {[llength $deps] > 1} {
				...
				lappend newisreg 1
				job multireg-[job_relfile2name $target] -deps $deps -targets {
					$target
				} -vars {
