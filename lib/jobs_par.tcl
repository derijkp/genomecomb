proc job_process_par_jobid {job} {
	if {![file_or_link_exists [job.file jid $job]]} {
		return ""
	}
...
proc job_logfile_par_close {} {
	...
	puts "All jobs are submitted."
	puts "You can follow the status based on the logfile $cgjob(logfile).running"
	puts "You can (manually, while still running) update the status (of jobs) in the log file using:"
	puts "cg job_update $cgjob(logfile).running"
	puts "When all jobs are finished, the log file will be automatically updated"
	puts "and renamed to $cgjob(logfile).finished after a succesful run"
	puts "or $cgjob(logfile).error in case there were jobs with errors"
	puts "In the latter case, you can use the following command for an overview of failed jobs and their error messages:"
	puts "cg error_report $cgjob(logfile).error"
}
