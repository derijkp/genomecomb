...
test compound {basic} {
	...
	foreach {test per samples criteria impact expected} {
		...
		cg compound -stack 1 -v 2 \
			-per $per -samples $samples -geneset test -criteria $criteria -impact $impact \
			tmp/atest.tsv tmp/result.tsv \
			>@ stdout 2>@ stderr
...
test compound {phasing} {
	write_tab tmp/vars.tsv {
		chromosome	begin	end	type	ref	alt	genotypes-a1-sample1	var	sequenced-a1-sample1 quality-a1-sample1 coverage-a1-sample1 sequenced-a1-sample2	genotypes-a1-sample2	quality-a1-sample2 coverage-a1-sample2 sequenced-a2-sample2	genotypes-a2-sample2 quality-a2-sample2 coverage-a2-sample2
		chr1	11600	11601	snp	C	A	1,0	v1	v	40	20	v	0;1	15	20	v	0,1	15	20
		chr1	12000	12001	snp	T	C	0,1	v2	v	50	25	v	0;1	20	30	v	0,1	20	30
		chr1	21600	21601	snp	A	C	1,0	v3	v	60	30	r	1;0	25	40	v	1,0	25	40
		chr1	22000	22001	del	G	{}	0,1	v4	v	70	50	v	1;0	30	50	v	1,0	30	50
	}
	write_tab tmp/gene_test.tsv {
		chrom	start	end	name	strand	cdsStart	cdsEnd	exonCount	exonStarts	exonEnds	name2
		chr1	11500	12100	cds1	+	11500	12050	1	11500,11900	11800,12100	cdsgene1
	...
	foreach {test per samples criteria impact expected} {
		test1 analysis {} {$sequenced == "v" and $quality >= 15 and $coverage >= 20} { >=RNA} {
			var	transcripts	compound-a1-sample1	compound-a1-sample2	compound-a2-sample2	compoundphase-a1-sample1	compoundphase-a1-sample2	compoundphase-a2-sample2
			v1	cdsgene1:cds1	2	2	2	b	u	2
			v2	cdsgene1:cds1	2	2	2	b	u	2
			v3	cdsgene3:cds3	2		2	b		1
			v4	cdsgene3:cds3	2		2	b		1
		}
		test2 analysis {} {$sequenced == "v" and $quality >= 15 and $coverage >= 20} { >=CDSMIS} {
			var	transcripts	compound-a1-sample1	compound-a1-sample2	compound-a2-sample2	compoundphase-a1-sample1	compoundphase-a1-sample2	compoundphase-a2-sample2
			v3	cdsgene3:cds3	2		2	b		1
		...
		test3 analysis {} {$sequenced == "v" and $quality >= 20 and $coverage >= 20} { >=RNA} {
			var	transcripts	compound-a1-sample1	compound-a1-sample2	compound-a2-sample2	compoundphase-a1-sample1	compoundphase-a1-sample2	compoundphase-a2-sample2
			v1	cdsgene1:cds1	2			b		
			v2	cdsgene1:cds1	2			b		
			v3	cdsgene3:cds3	2		2	b		1
			v4	cdsgene3:cds3	2		2	b		1
		}
		test3samples analysis {a1-sample2 a2-sample2} {$sequenced == "v" and $quality >= 20 and $coverage >= 20} { >=RNA} {
			var	transcripts	compound-a1-sample2	compound-a2-sample2	compoundphase-a1-sample2	compoundphase-a2-sample2
			v3	cdsgene3:cds3		2		1
			v4	cdsgene3:cds3		2		1
		}
		test3samplesdup analysis {a2-sample2 a2-sample2} {$sequenced == "v" and $quality >= 20 and $coverage >= 20} { >=RNA} {
			var	transcripts	compound-a2-sample2	compoundphase-a2-sample2
			v3	cdsgene3:cds3	2	1
			v4	cdsgene3:cds3	2	1
		}
		test4 analysis {} {$sequenced == "v" and $quality >= 100 and $coverage >= 20} { >=CDSMIS} {
		...
		test1s sample {} {$sequenced-a1 == "v" and $quality-a1 >= 15 and $coverage-a1 >= 20} { >=RNA} {
			var	transcripts	compound-sample1	compound-sample2	compoundphase-sample1	compoundphase-sample2
			v1	cdsgene1:cds1	2	2	b	u
			v2	cdsgene1:cds1	2	2	b	u
			v3	cdsgene3:cds3	2		b	
			v4	cdsgene3:cds3	2		b	
		}
		test1s sample sample2 {$sequenced-a1 == "v" and $quality-a1 >= 15 and $coverage-a1 >= 20} { >=RNA} {
			var	transcripts	compound-sample2	compoundphase-sample2
			v1	cdsgene1:cds1	2	u
			v2	cdsgene1:cds1	2	u
		}
	} {
		puts -nonewline "$test: "
		set expected [deindent $expected]
		# putsvars expected criteria impact
		if {$per eq "sample"} {
			set phasing genotypes-a1
		} else {
			set phasing genotypes
		}
		cg compound -stack 1 -v 2 \
			-phasing $phasing -per $per -samples $samples -geneset test -criteria $criteria -impact $impact \
			tmp/atest.tsv tmp/result.tsv \
			>@ stdout 2>@ stderr
		set result [exec cg select -f {var transcripts compound-* compoundphase-*} tmp/result.tsv]
		if {$result ne $expected} {
			error "different result for $test:\n$result\nshould be:\n$expected"
		} else {
			puts "ok"
