...
test bcol_index {basic} {
	...
	exec cg bcol get tmp/temp.sft.index/lines.bcol 0 2
} {76 103 228} 
...
test bcol_make {-p} {
	...
	catch {exec cg bcol make -p pos tmp/temp coverage < data/cov.tsv}
	exec cg bcol table tmp/temp.bcol 0 49 > tmp/temp.test
	exec cg select -f {pos	coverage} -q {$chromosome == "chr1"} data/cov.tsv tmp/temp.test2
	catch {exec diff tmp/temp.test tmp/temp.test2} e
	regsub {child process exited abnormally} $e {} e
	string trim $e
} {1c1
