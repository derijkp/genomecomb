...
test genome_seq {basic} {
	exec cg genome_seq -i name data/reg_genome_seq.tsv /complgen/refseq/hg19_test > tmp/temp.fas
	exec diff tmp/temp.fas data/expected-reg_genome_seq.fas
} {} 

test genome_seq {basic -f -1 -n -1} {
	exec cg genome_seq -i name -f -1 -n -1 data/reg_genome_seq.tsv /complgen/refseq/hg19_test > tmp/temp.fas
	exec diff tmp/temp.fas data/expected-reg_genome_seq_fullmasked.fas
} {} 

test genome_seq {basic -p {snp 1000g}} {
	exec cg genome_seq -i name -f -1 -n -1 -p {snp 1000g} data/reg_genome_seq.tsv /complgen/refseq/hg19_test > tmp/temp.fas
	exec diff tmp/temp.fas data/expected-reg_genome_seq_fullmasked.fas
} {} 

test genome_seq {basic -p {1000g}} {
	exec cg genome_seq -i name -f -1 -n -1 -p {1000g} data/reg_genome_seq2.tsv /complgen/refseq/hg19_test > tmp/temp.fas
...
test genome_seq {concat and makemap} {
	exec cg genome_seq -c "" -m tmp/temp.map -i name data/reg_genome_seq.tsv /complgen/refseq/hg19_test > tmp/temp.fas
...
test genome_seq {gcsplit} {
	exec cg genome_seq -g 50 -gs 60 -i name data/reg_genome_seq.tsv /complgen/refseq/hg19_test tmp/temp.fas
...
test genome_seq {-g 0} {
	exec cg genome_seq -g 0 data/reg_genome_seq.tsv /complgen/refseq/hg19_test tmp/temp.fas
	exec diff tmp/temp.fas data/expected-reg_genome_seq-noid-g0.fas
} {}

test genome_seq {-g 0 -gd 0} {
	exec cg genome_seq -g 0 -gd 0 data/reg_genome_seq.tsv /complgen/refseq/hg19_test tmp/temp.fas
	exec diff tmp/temp.fas data/expected-reg_genome_seq-noid.fas
} {}

test genome_seq {gcsplit and -l} {
	exec cg genome_seq -g 50 -gs 60 -l _ -i name data/reg_genome_seq.tsv /complgen/refseq/hg19_test tmp/temp.fas
