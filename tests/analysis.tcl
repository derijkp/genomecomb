...
test exportplink {basic} {
	test_cleantmp
	exec cg exportplink data/vars3.tsv tmp/temp
...
test exportplink {compressed} {
	test_cleantmp
	cg zst -o tmp/vars3.tsv.zst data/vars3.tsv
...
test exportplink {del} {
	test_cleantmp
	set c [file_read data/vars3.tsv]
...
test exportplink {codegeno} {
	test_cleantmp
	exec cg exportplink -c 1 data/vars3.tsv tmp/temp
	exec diff tmp/temp.tfam.pre data/expected-vars3.tfam.pre
...
test exportplink {samples} {
	...
	exec cg exportplink -s sample2 data/vars3.tsv tmp/temp
...
test exportplink {names with -} {
	...
	set header [cg select -h data/vars3.tsv]
	set header [string_change $header {-sample1 -m1-sample1 -sample2 -m2-sample1}]
	cg select -nh $header data/vars3.tsv tmp/tempsrc.tsv
...
test exportplink {query} {
	test_cleantmp
	exec cg exportplink -q {$chromosome == "chr1"} data/vars3.tsv tmp/temp
	exec diff tmp/temp.tped data/expected-vars3.tped
} {6a7,8
...
test exportplink {query error} {
	test_cleantmp
	exec cg exportplink -q {$bla == "chr1"} data/vars3.tsv tmp/temp
	exec diff tmp/temp.tped data/expected-vars3.tped
