...
test primercheck {freqp} {
	cg cplinked $::refseqdir/hg19 tmp
	file delete {*}[glob tmp/hg19/var_hg19_snp135.*]
	cg select -f {chrom start end type ref alt name {freqp=catch($freq * 100.0,$freq)} avHetSE strand molType valid func weight exceptions submitterCount submitters alleleFreqCount alleles alleleNs alleleFreqs bitfields} data/var_hg19_partofsnp135.tsv.zst tmp/hg19/var_hg19_snp135.tsv.zst
	cg zstindex tmp/hg19/var_hg19_snp135.tsv.zst
	cg maketabix tmp/hg19/var_hg19_snp135.tsv.zst
	exec cg primercheck data/primers.tsv tmp/hg19 tmp/primerinfo.tsv
	exec diff tmp/primerinfo.tsv data/primercheck-results.tsv
