...
color_palette = sns.xkcd_palette(['windows blue', 'faded green', 'dusty purple', 'coffee', 'amber', 'faded red', 'very light purple', 'milk chocolate'])
hex_colors = ['#F3C300','#875692','#F38400','#A1CAF1','#BE0032','#C2B280','#848482','#008856','#E68FAC','#0067A5','#F99379','#604E97','#F6A600','#B3446C','#DCD300','#882D17','#8DB600','#654522','#E25822','#2B3D26']
...
proportions = proportions_color + [gray_bar[0]]
...
sortedisoforms=[]
for name in list(keepiso.keys()):
    for line in isoforms:
        if line[4] == name:
            sortedisoforms.append(line)
            break

#isoforms = sorted(isoforms,key=lambda x: x[3], reverse=True)  # sort by productivity
packed = pack(sortedisoforms, rev=False, tosort=False)
