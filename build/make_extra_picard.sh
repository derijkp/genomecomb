...
# This script builds portable picard binaries using the Holy build box environment
# options:
...
mamba create -y -n picard
mamba activate picard
conda config --add channels defaults
conda config --add channels bioconda
conda config --add channels conda-forge
mamba install -y picard=$picardversion python=3.12
...
rm picard.tar.gz || true
conda pack -n picard -o picard.tar.gz
rm -rf picard-$picardversion-$arch.old
mv picard-$picardversion-$arch picard-$picardversion-$arch.old || true
mkdir /build/picard-$picardversion-$arch
cd /build/picard-$picardversion-$arch
tar xvzf ../picard.tar.gz
...
cd /build
rm picard.tar.gz
ln -sf picard-$picardversion-$arch/picard picard
ln -sf picard-$picardversion-$arch/picard picard-$picardversion
tar cvzf picard-$picardversion-$arch.tar.gz picard-$picardversion-$arch picard picard-$picardversion
cp -ra picard-$picardversion-$arch picard picard-$picardversion /io/extra$ARCH
cd /io/extra$ARCH/

echo "Finished building picard-$picardversion-$arch"
echo "build in $builddir/picard-$picardversion-$arch"
echo "installed in $srcdir/picard-$picardversion-$arch"
