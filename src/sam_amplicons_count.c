...
int main(int argc, char *argv[]) {
	...
	DString *line1 = NULL,*line2 = NULL,*linekeep = NULL,*linetemp = NULL,*empty=NULL,*seq=NULL;
	DString *chromosome1 = NULL,*curchromosome = NULL,*chromosomekeep = NULL;
	ssize_t read;
	unsigned int curpos=0, flag;
	int comp,chr2pos,start2pos,end2pos,outerstart2pos,outerend2pos,namepos,max2,i;
	...
	int error2,reverse;
	if ((argc != 8)) {
		fprintf(stderr,"Format is: sam_clipamplicons ampliconsfile chrpos startpos endpos outerstartpos outerendpos namepos");
		exit(EXIT_FAILURE);
	}
	for (i = 0 ; i < AMPLICONSBUFFERSIZE ; i++) {
		amplicon[i].chr2 = DStringNew();
		amplicon[i].name = DStringNew();
	}
	cigar.size = 0;
	cigar.memsize = 0;
	...
	if (!error2) {
		...
		if (namepos != -1) {
			DStringCopy(amplicon[0].name,result2->data+namepos);
		}
	...
	fprintf(stdout,"chromosome\tbegin\tend\tname\tcount\n");
	while (1) {
		pos1++;
		...
		while (!error2) {
			...
			if (namepos != -1) {
				DStringCopy(amplicon[ampcur].name,result2->data+namepos);
			}
		...
		while (ampnum) {
			...
			fprintf(stdout,"%s\t%d\t%d\t%s\t%d\n",amplicon[amppos].chr2->string,amplicon[amppos].start2-1,amplicon[amppos].end2-1,amplicon[amppos].name->string,amplicon[amppos].count);
			amppos++; if (amppos >= AMPLICONSBUFFERSIZE) {amppos = 0;}
			ampnum--;
		}
		ampcur = amppos;
	...
	count = ampnum;
	while (count--) {
		fprintf(stdout,"%s\t%d\t%d\t%s\t%d\n",amplicon[ampcur].chr2->string,amplicon[ampcur].start2-1,amplicon[ampcur].end2-1,amplicon[ampcur].name->string,amplicon[ampcur].count);
		ampcur++; if (ampcur >= AMPLICONSBUFFERSIZE) {ampcur -= AMPLICONSBUFFERSIZE;}
	}
	while (error2) {
		error2 = DStringGetTab(line2,f2,max2,result2,1,&numfields);	pos2++;
