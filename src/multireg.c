...
int multireg_next(FILE *f1,DString *line1,int chr1pos, int start1pos, int end1pos, int max1,DString *result1,char **chromosome1,int *nchr1,int *start1,int *end1,DString **cur1) {
	int error1;
...
int main(int argc, char *argv[]) {
	...
	char *chromosome1,*chromosome2,*nulldata;
...
NODPRINT("%d %d %d ""%s"" %d %d %d",chr1pos,start1pos,end1pos,nulldata,chr2pos,start2pos,end2pos)
	...
	error1 = multireg_next(f1,line1,chr1pos,start1pos,end1pos,max1,result1,&chromosome1,&nchr1,&start1,&end1,&cur1);
	error2 = multireg_next(f2,line2,chr2pos,start2pos,end2pos,max2,result2,&chromosome2,&nchr2,&start2,&end2,NULL);
	while (!error1 || !error2) {
NODPRINT("%d:%d-%d %s",nchr1,start1,end1,cur1->string)
NODPRINT("%d:%d-%d",nchr2,start2,end2)
		if (nchr1 > curchr || nchr2 > curchr) {curchr = (nchr1!=1000)?nchr1:nchr2; nextpos = 0;}
		if (start1 >= nextpos || start2 >= nextpos) {
			fprintf(stderr, "%s-%d\n",chromosome1,nextpos);
			fflush(stderr);
			nextpos += 10000000;
		}
		if ((nchr1 == 1000) && (nchr2 == 1000)) break;
		if ((nchr2 < nchr1) || (nchr1 == nchr2 && end2 <= start1)) {
			fprintf(stdout,"%s\t%d\t%d\t%s\t%d\n",chromosome2,start2,end2,nulldata,1);
			error2 = multireg_next(f2,line2,chr2pos,start2pos,end2pos,max2,result2,&chromosome2,&nchr2,&start2,&end2,NULL);
		} else if ((nchr1 < nchr2) || (nchr1 == nchr2 && end1 <= start2)) {
			fprintf(stdout,"%s\t%d\t%d\t%s\t%d\n",chromosome1,start1,end1,cur1->string,0);
			error1 = multireg_next(f1,line1,chr1pos,start1pos,end1pos,max1,result1,&chromosome1,&nchr1,&start1,&end1,&cur1);
		} else {
			if (start1 < start2) {
				fprintf(stdout,"%s\t%d\t%d\t%s\t%d\n",chromosome1,start1,start2,cur1->string,0);
				start1 = start2;
			}
			if (start2 < start1) {
				fprintf(stdout,"%s\t%d\t%d\t%s\t%d\n",chromosome1,start2,start1,nulldata,1);
				start2 = start1;
			}
			if (end2 < end1) {
				fprintf(stdout,"%s\t%d\t%d\t%s\t%d\n",chromosome1,start1,end2,cur1->string,1);
				start1 = end2;
				error2 = multireg_next(f2,line2,chr2pos,start2pos,end2pos,max2,result2,&chromosome2,&nchr2,&start2,&end2,NULL);
			} else if (end1 < end2) {
				fprintf(stdout,"%s\t%d\t%d\t%s\t%d\n",chromosome1,start1,end1,cur1->string,1);
				start2 = end1;
				error1 = multireg_next(f1,line1,chr1pos,start1pos,end1pos,max1,result1,&chromosome1,&nchr1,&start1,&end1,&cur1);
			} else {
				fprintf(stdout,"%s\t%d\t%d\t%s\t%d\n",chromosome1,start1,end1,cur1->string,1);
