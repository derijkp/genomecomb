	...
	while ((read = getline(&line, &len, stdin)) != -1) {
		...
		if (last) {
			if (start2 == -1) {
				...
				fprintf(stdout,"%d\t%d\t%c\t%d\t%d\t%d\t%d\t%c",
					chr1,bin,strand1,start1,end1,weight1,numl,'s');
				...
				fprintf(stdout,"%d\t%d\t%c\t%d\t%d\t%d\t%d\t%c",
					chr1,bin,strand1,start1,end1,weight1,numl,type);
				fprintf(stdout,"\t%d\t%c\t%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\n",
					chr2,strand2,start2,end2,weight2,numr,dist,num,fnum,(side1)?1:0);
				if (type == 'c') {
					bin = pos2bin(start2,end2);
					fprintf(stdout,"%d\t%d\t%c\t%d\t%d\t%d\t%d\t%c",
						chr2,bin,strand2,start2,end2,weight2,numr,type);
					fprintf(stdout,"\t%d\t%c\t%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\n",
						chr1,strand1,start1,end1,weight1,numl,dist,num,fnum,(side2)?1:0);
				}
			}
			fnum++;
			cur = 0;
